...
def cmd_benchmark(args: argparse.Namespace) -> None:
    ...
    with open(summary_md, "w", encoding="utf-8") as f:
        ...
        f.write("## 1. K-Means Alt-├ûrnekleme Performans K─▒yaslamas─▒ (512x512)\n\n")
        f.write("| ├ûrnekleme Boyutu | Ortalama S├╝re (ms) | Throughput (FPS) | ─░nertia |\n")
        f.write("| :--- | :--- | :--- | :--- |\n")
        for kr in kmeans_results:
            f.write(f"| {kr['subsample_size']} | {kr['mean_ms']} ms | {kr['throughput_fps']} FPS | {kr['inertia']} |\n")
        f.write("\n## 2. Metrik Hesaplama H─▒z─▒ (10,000 Renk ├çifti)\n\n")
        f.write(f"- **CIE 1976 (├ûklid):** {cie76_ms:.2f} ms ({int(num_pairs/(cie76_ms/1000.0)):,} ├ğift/sn)\n")
        f.write(f"- **CIEDE2000:** {ciede00_ms:.2f} ms ({int(num_pairs/(ciede00_ms/1000.0)):,} ├ğift/sn)\n\n")
        f.write("## 3. Sharma et al. (2005) Standart Do─şrulama ├çiftleri\n\n")
